...
const calculateSalary = () => {
      const totalSalary =allowance+basicSalary+ (overtime * 0.1 * basicSalary) - (basicSalary / 30 * (30 - attendance)) ;
...
function sendData(e){
  ...
  axios.post("/salary/added",newSalary).then(()=>{
     alert("Salary Added")
     setEmpId("");
        setBasicsalary(0);
        setAllowance(0);
    ...
    <div><div><button className="addbutton">Add</button></div></div>
    </div>
